...
Every tunable parameter, its source file, and its default. Produced during the S07 config lockdown.
...
| Provider | `deepinfra` | `config/budgets.json` `.model.provider` | The search adapter search itself is HTTP, not LLM. |
...
|-----------|-------|--------|------|
| Max iterations | `3` | `config/budgets.json` `.coverage.max_iterations` | Hard cap. The coordinator, not the model, decrements. |
...
| Parameter | Value | Source | Note |
|-----------|-------|--------|------|
| Allowed domains | `[]` (all) | `config/domain_policy.json` `.allow_list` | Empty means no restriction. |
| Blocked domains | `[]` | `config/domain_policy.json` `.deny_list` | Add malicious or low-trust domains here. |

## Slot taxonomy

| Parameter | Value | Source | Note |
|-----------|-------|--------|------|
| Slot definitions | 9 slots (overview, products, funding, headcount, tech_stack, hiring, leadership, recent_news, risks) | `config/slot_taxonomy.json` | Required vs optional and confidence floors. |
...
| Auth stale window | `300 seconds` | `src/hmac.ts` | Requests signed more than 5 min ago are rejected. |

## Hardcoded values found (kept on purpose and documented)

These are intentionally hardcoded. They are architectural invariants, not tunable parameters.
...
| 200-char log truncation | `src/log.ts` | n8n Cloud log retention limit. |
